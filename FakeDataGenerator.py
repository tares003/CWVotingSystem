...
with open("RandomStudents.csv", "w", newline='') as file:
    writer = csv.DictWriter(file, fieldnames=["name", "Registered", "dob", "id"], quoting=csv.QUOTE_ALL)
...
with open("RandomCandidates.csv", "w", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=["name", "Registered", "dob", "id", "position"], quoting=csv.QUOTE_ALL)
    header_written = False
